...
<style>
.verification-overlay { position: fixed; inset: 0; background: rgba(255,255,255,0.85); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 1rem; }
.verification-block-modal { background: #ffffff; border-radius: 28px; box-shadow: 0 20px 60px rgba(60,80,50,0.2); padding: 2.5rem; text-align: center; max-width: 440px; width: 100%; }
.verification-block-icon { width: 64px; height: 64px; margin: 0 auto 1.25rem; border-radius: 50%; background: #fef3d0; display: grid; place-items: center; font-size: 1.8rem; }
.verification-block-title { font-family: 'Lora', serif; font-size: 1.4rem; font-weight: 700; color: #2c3028; margin-bottom: 0.75rem; }
.verification-block-text { font-size: 0.92rem; color: #7a8270; line-height: 1.7; margin-bottom: 1.75rem; }
.verification-block-actions { display: flex; justify-content: center; }
.verification-block-btn { padding: 0.75rem 1.5rem; border-radius: 18px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.88rem; font-weight: 700; border: none; cursor: pointer; background: #5a8a68; color: #fff; }
.verification-block-btn:hover { background: #3d6b50; }
</style>
<?php endif; ?>
